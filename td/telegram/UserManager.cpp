  ...
  if (id == telegram_api::userStatusOnline::ID) {
    ...
    if (new_online >= now) {
      if (new_online > now + 10) {
        // this is possible when local time isn't adjusted yet after hibernation, but updates were received
        LOG(INFO) << "Receive userStatusOffline from the future time " << new_online << ", now is " << now;
      }
      new_online = now - 1;
    }
    is_offline = true;
...
void UserManager::on_update_user_noforwards(UserId user_id, bool noforwards_my_enabled, bool noforwards_peer_enabled) {
  ...
  UserFull *user_full = get_user_full_force(user_id, "on_update_user_noforwards");
  if (user_full == nullptr) {
    return;
  }
...
bool UserManager::get_user_has_protected_content_force(UserId user_id, bool only_my) {
...
void UserManager::on_get_user_full(telegram_api::object_ptr<telegram_api::userFull> &&user) {
  ...
  if (is_bot && (!td_->auth_manager_->is_bot() || user_id == get_my_id())) {
    auto *bot_info = user_full->add_bot_info();
...
td_api::object_ptr<td_api::userFullInfo> UserManager::get_user_full_info_object(UserId user_id,
  ...
  if (is_bot) {
    ...
    bio_object = get_formatted_text_object(this, bio, true, -1);
  }
  ...
  return td_api::make_object<td_api::userFullInfo>(
      ...
      user_full->noforwards_my_enabled, std::move(bio_object), user_full->birthdate.get_birthdate_object(),
